...
def update(data: dict, cameraDisplay):
    ...
    printer.print(window, f" LX: {round(data["joystickValues"]["sticks"]["lx"], 2): .2f}  | R:  {round(data["motors"]["RIGHT"], 2)}")  # round the display values to 2 decimal points
    printer.print(window, f" LY: {round(data["joystickValues"]["sticks"]["ly"], 2): .2f}  | L:  {round(data["motors"]["LEFT"], 2)}")  # with the ": .2f", it turns the - to a space with the " ", and ensures 2 decimal points with ".2f"
    printer.print(window, f" RX: {round(data["joystickValues"]["sticks"]["rx"], 2): .2f}  | NW: {round(data["motors"]["NW"], 2)}")
    printer.print(window, f" RY: {round(data["joystickValues"]["sticks"]["ry"], 2): .2f}  | NE: {round(data["motors"]["NE"], 2)}")
    printer.print(window, f"            | SW: {round(data["motors"]["SW"], 2)}")
    printer.print(window, f"            | SE: {round(data["motors"]["SE"], 2)}")
    printer.outdent()
    printer.print(window, "")
    printer.print(window, "")
